...
public partial class MainForm : MetroFramework.Forms.MetroForm
{
    ...
    private void metroButton83_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (DiscordChannel discordChannel in discordGuild.GetChannels())
            {
                try
                {
                    if ((metroCheckBox108.Checked && discordChannel.Type == 0) || (metroCheckBox109.Checked && (int)discordChannel.Type == 2) || (metroCheckBox110.Checked && (int)discordChannel.Type == 4) || (metroCheckBox114.Checked && (int)discordChannel.Type == 5))
                    {
                        try
                        {
    ...
    private void metroButton136_Click(object sender, EventArgs e)
    {
        try
        {
            getIndexedGuild(metroComboBox4.SelectedIndex).GetUser(theSocketClient.CurrentUser.Id).ModifyAsync(x =>
            {
                x.Nickname = metroTextBox69.Text;
            });
        }
        catch (Exception)
        {
    ...
    private void metroButton170_Click(object sender, EventArgs e)
    {
        try
        {
            foreach (PartialGuild guild in discordClient.GetGuilds())
            {
                try
                {
                    guild.ChangeClientNickname(metroTextBox81.Text);
                }
                catch (Exception)
                {
                }
    ...
    private void metroButton171_Click(object sender, EventArgs e)
    {
        try
        {
            foreach (PartialGuild guild in discordClient.GetGuilds())
            {
                try
                {
                    guild.ChangeClientNickname(discordClient.GetClientUser().Username);
                }
                catch (Exception)
                {
                }
